...
func init() {
	ConnectionServiceCmd.AddCommand(GetIamPolicyCmd)
...
var GetIamPolicyCmd = &cobra.Command{
	Use:   "get-iam-policy",
	Short: "Gets the access control policy for a resource. ...",
	Long:  "Gets the access control policy for a resource.  Returns an empty policy if the resource exists and does not have a policy  set.",
	...
	RunE: func(cmd *cobra.Command, args []string) (err error) {
		...
		if Verbose {
			printVerboseInput("Connection", "GetIamPolicy", &GetIamPolicyInput)
		}
		resp, err := ConnectionClient.GetIamPolicy(ctx, &GetIamPolicyInput)
